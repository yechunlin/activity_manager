...
##### 4.1 提取报名字段 `fields`（复用 step_2 规则）
...
使用 iframe 呈现：
```html
<iframe src="preview_url" width="320px" height="580px"></iframe>
```
...
- Step4：用户确认（必须执行）
  ...
  - 询问：
  > 为您匹配到一个高度契合的模板，已提供预览效果，请查看后确认是否使用该模板创建活动？
...
```
IF fields 已提取 AND post_img 已提取
→ 跳过 step_2、step_3、step_4

ELSE IF fields 已提取 AND post_img 未提取
→ 跳过 step_2、step_3，进入 step_4

ELSE IF fields 未提取 AND post_img 已提取
→ 跳过 step_4，执行 step_2、step_3

ELSE
→ 正常执行 step_2 → step_3 → step_4
```
...
**必须提供**：预设风格选项列表（来自 `config/designh5/scheme_conf.json`）
- 格式如下：
```json
...
- **⚠️ 关键交互：内置海报确认**
  1. **告知用户**：
     > "该风格有配套的内置海报，会覆盖已提供的和程序生成的海报，是否使用？（是 / 否）"
  2. **用户选择"是"**：
     - 记录对应`use_default_post`的值为`1`
  3. **用户选择"否"**：
     - 记录对应`use_default_post`的值为`0`
- 不生成 `scheme` JSON（使用预设配色）
- 进入下一步

#### 情况 2：用户选择"自动生成"

**触发语义**："自动生成""你帮我配""随便配一个"、未选择具体风格
...
- ✅ `tag_id` 和 `scheme` **二选一必须存在**：
  ...
  - 有 `tag_id` → `use_default_post` 为`1`|`0`
